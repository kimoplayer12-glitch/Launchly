...
if (!fs.existsSync(serviceAccountPath)) {
  ...
  console.log('\nPlease ensure your Firebase service account JSON is placed at the root directory.');
  process.exit(1);
}
...
async function findUserByName(displayName: string) {
  ...
  try {
    ...
    for (const [uid, userData] of Object.entries(users)) {
      ...
      if (user.displayName === displayName || user.displayName?.includes(displayName)) {
...
async function addCreditsToUser(uid: string, amount: number) {
